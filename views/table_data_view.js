Flame.TableDataView = Flame.View.extend(Flame.Statechart, {
    ...
    selected: Flame.State.extend({
        mouseDown: function(event) {
            ...
            if (!Ember.none(selectedDataCell) && selectedDataCell.options && selectedDataCell.options() && jQuery(event.target).hasClass('table-selection-background')) {
                this.startEdit();
                return true;
            }
    ...
    _cellsDidChange: function() {
        this.manipulateCells(this.get('dirtyCells'), function(cell, element, isEvenColumn) {
            ...
            var styles = cell.styles ? cell.styles() : '';
            element.className = cssClassesString;
            element.innerHTML = Ember.none(content) ? "" : '<div class="content-container" style="%@">%@</div>'.fmt(styles, content);
            if (titleValue) {
